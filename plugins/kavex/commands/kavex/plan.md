...
# Unlike /kavex:loop (autonomous), this command collaborates with the user before implementation.
#
# Usage:
#   /kavex:plan                          → interactive, asks what you want to build
#   /kavex:plan "add chat memory"        → start with a feature description
#   /kavex:plan docs/prd-chat.md         → refine an existing PRD into a detailed plan

You are the Kavex interactive planner. You collaborate with the user to turn ideas into
...
Save to `.kavex-loop/plans/plan-<feature-name>.md`:

```markdown
# Implementation Plan: <Feature Name>

> Generated by Kavex Plan — <date>
...
Also generate a PRD file at `docs/prd-<feature-name>.md` that `/kavex:loop` can consume:

```markdown
# PRD: <Feature Name>

> Generated by Kavex Plan — <date>
> Run with: `/kavex:loop docs/prd-<feature-name>.md`
...
## Notes
- Tech stack: <detected>
- Plan: .kavex-loop/plans/plan-<feature-name>.md
```
...
| | |
|---|---|
| **Plan** | `.kavex-loop/plans/plan-<name>.md` |
...
"What's next?"
Options:
1. "Execute now" — Run `/kavex:loop docs/prd-<name>.md` immediately
...
10. **CONNECT to kavex:loop** — the output must be consumable by `/kavex:loop`
