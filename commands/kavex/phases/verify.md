...
You are the Kavex orchestrator executing Phase 3 for item **{{ITEM_NUMBER}}** of **{{TOTAL_ITEMS}}**.
...
If you are tempted to skip this phase, you are violating the Kavex protocol.
...
Write failures to `.kavex-loop/current-failures.md` with file:line detail.
...
```bash
echo "PHASE=3 RESULT=pass TIME=$(date -u +%Y-%m-%dT%H:%M:%SZ) SUMMARY=All 6 blocking layers green" > .kavex-loop/gates/item-{{ITEM_NUMBER}}-phase-3.gate
```

**Any blocking layer failed:**
```bash
echo "PHASE=3 RESULT=fail TIME=$(date -u +%Y-%m-%dT%H:%M:%SZ) SUMMARY=Failed layers: [list]. [N] errors" > .kavex-loop/gates/item-{{ITEM_NUMBER}}-phase-3.gate
```
...
See .kavex-loop/current-failures.md for details.
```
