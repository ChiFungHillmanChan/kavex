...
You are the Kavex orchestrator executing Phase 4 for item **{{ITEM_NUMBER}}** of **{{TOTAL_ITEMS}}**.
...
```bash
cat .kavex-loop/gates/item-{{ITEM_NUMBER}}-phase-3.gate
```
...
```bash
git diff HEAD -- [changed files] > /tmp/kavex-review-diff.txt
```
...
## The diff
[contents of /tmp/kavex-review-diff.txt]
...
3. Write to `.kavex-loop/current-review.md`:
...
```bash
echo "PHASE=4 RESULT=fail TIME=$(date -u +%Y-%m-%dT%H:%M:%SZ) SUMMARY=[N] HIGH findings found" > .kavex-loop/gates/item-{{ITEM_NUMBER}}-phase-4.gate
```
```
...
```bash
echo "PHASE=4 RESULT=pass TIME=$(date -u +%Y-%m-%dT%H:%M:%SZ) SUMMARY=[N] findings (0 HIGH, M MEDIUM, L LOW)" > .kavex-loop/gates/item-{{ITEM_NUMBER}}-phase-4.gate
```
```
