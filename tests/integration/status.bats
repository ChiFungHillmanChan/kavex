...
@test "status: runs without error on fresh install" {
  run_install "$SANDBOX"
  run run_kova "$SANDBOX" status
  assert_success
  assert_output --partial "KOVA STATUS"
}
...
@test "help: displays without error" {
  run run_kova "$SANDBOX" help
  assert_success
  assert_output --partial "KOVA PROTOCOL"
}
