...
@test "kova-monitor help: shows help text" {
  run bash "$KOVA_ROOT/scripts/kova-monitor" help
  assert_success
...
@test "kova-monitor: no args shows help" {
  run bash "$KOVA_ROOT/scripts/kova-monitor"
  assert_success
  assert_output --partial "KOVA MONITOR"
}
...
@test "kova-monitor: unknown command fails" {
  run bash "$KOVA_ROOT/scripts/kova-monitor" foobar
  assert_failure
...
@test "kova-monitor start: requires prd file argument" {
  run bash "$KOVA_ROOT/scripts/kova-monitor" start
  assert_failure
  assert_output --partial "PRD file required"
}

@test "kova-monitor start: fails on missing prd file" {
  run bash "$KOVA_ROOT/scripts/kova-monitor" start /nonexistent/file.md
  assert_failure
...
@test "kova-monitor start: error when tmux not available" {
  ...
  run env PATH="/usr/bin:/bin" bash "$KOVA_ROOT/scripts/kova-monitor" start "$SANDBOX/test.md"
...
@test "kova-monitor status: runs without error" {
  run bash "$KOVA_ROOT/scripts/kova-monitor" status
  assert_success
}
...
@test "kova-monitor stop: no error when no session exists" {
  run bash "$KOVA_ROOT/scripts/kova-monitor" stop
