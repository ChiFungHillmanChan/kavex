...
<p align="center">
  <img src="../../assets/kavex-hero.png" alt="Kavex — 自主工程协议" width="100%" />
</p>
...
Kavex 是一套"规则 + 脚本 + 命令"的组合包。你安装到任何一个项目里，Claude Code 就会从一个"你问它答"的 AI 助手，变成一个"自动开发、自动测试、自动修复"的工程团队。

**简单说：安装了 Kavex 之后，Claude 不再问你"我该不该这样做？"，它直接做，做完自己检查，检查不过自己再改。**
...
```bash
claude /install kavex          # 轻量版：命令 + 技能
claude /install kavex-full     # 完整版：命令 + 技能 + hooks + 执行保障
```
...
|------|----------|
| **kavex** | 斜杠命令 + 工程协议技能 |
| **kavex-full** | kavex 全部内容 + 安全 hooks、验证闸门、自动格式化、commit 闸门、Team Loop |
...
<p align="center">
  <img src="../../assets/kavex-safety.png" alt="Kavex 7 层验证架构" width="100%" />
</p>
...
完整的 7 层验证是在 **Team Loop**（`/kavex:loop`）里通过 `verify-gate.sh` 运行的，不是每次停都跑。包括：
...
<p align="center">
  <img src="../../assets/kavex-workflow.png" alt="Kavex Team Loop — 6 阶段工作流程" width="100%" />
</p>

这是 Kavex 最强大的功能。你写一个 PRD（Product Requirements Document，就是一个待办清单），然后输入：
...
**但它不会问你。** 它会自己做假设，然后写到 `.kavex-loop/plans/item-N-clarify.md`。
...
4. 写一个详细计划到 `.kavex-loop/plans/item-N-plan.md`
...
**如果任何一层失败：**
- 将所有错误写入 `.kavex-loop/current-failures.md`，包含 file:line 的具体位置
...
写入 `.kavex-loop/current-review.md`，格式：
```
...
- `.kavex-loop/LOOP_PROGRESS.md` — 哪些项目完成了、哪个正在做
- `.kavex-loop/ITERATION_LOG.md` — 每次 iteration 的详细记录

**STUCK 检测：**
如果同一个项目修了 5 次（默认）都修不好，Claude 会：
- 标记为 STUCK
- 写入 `.kavex-loop/STUCK_ITEMS.md`
- 跳过，继续下一个

**可恢复：**
如果你中途停了（断线、手动取消等），`.kavex-loop/` 里面保存了所有进度。下次再运行 `/kavex:loop docs/my-prd.md`，它会问你：`resume`（继续）/ `restart`（重新开始）/ `cancel`（取消）。
...
做大型功能（多个项目）：
  /kavex:loop docs/prd.md      <- 自动逐项实现，每项过 6 个阶段
...
<p align="center">
  <img src="../../assets/kavex-comparison.png" alt="没有 Kavex vs 有 Kavex" width="100%" />
</p>
...
**Kavex 的哲学："你不是信任它会做对的事，你是建立一个系统让做错的事变得困难。"**
...
**Hooks 不保证的事情：**
- 用户可以停用 hooks（`kavex deactivate` 或者改 settings.json）
