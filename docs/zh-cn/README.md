...
- [四个 Hooks（自动触发）](#四个-hooks自动触发)
  ...
  - [Hook 2：7 层闸门](#hook-2verify-on-stopsh--7-层闸门)
...
### Hook 2：`verify-on-stop.sh` — 7 层闸门

**什么时候运行？** 每次 Claude 说"我做完了"想停下来的时候。

**做什么？** 运行 7 层检查。前 6 层是"阻止型"——不通过的话 Claude **不能停**，得继续修。第 7 层只是警告。
...
- Java → `mvn compile`
- .NET → `dotnet build`
- 如果 build 不过 → **阻止，Claude 不能停**

**第 2 层：Unit Tests（单元测试）**
- 运行你所有的 unit test。
- 自动检测测试框架：vitest、jest、pytest、go test、cargo test、rspec、mvn test……
- **Flaky test 处理：** 如果有 test 失败，它会自动重跑一次。第二次通过就算通过（因为有些 test 是偶尔失败的）。
- 如果重跑也失败 → **阻止**

**第 3 层：Integration Tests（集成测试）**
- 只有你的项目配置了 `test:integration` 脚本才会运行。
- 没有的话自动跳过。
- 有的话和 unit test 一样，失败会 retry 一次。

**第 4 层：E2E Tests（端到端测试）**
- 只有你安装了 Playwright 才会运行。
- 没有 Playwright → 自动跳过。
- 有的话运行 `npx playwright test`。
...
**第 7 层：Security Audit（安全审计）— 仅警告**
- `npm audit`、`pip-audit`、`cargo audit`……
- 有问题只会显示警告，**不会阻止**。

**也就是说：** Claude 做完了，想停下来，但它得过了 6 关才能停。如果任何一关失败，它得自己修到通过为止。
...
会保护的文件包括：
- `.env`、`.env.production`、`.env.local` — 环境变量（通常有 API key）
- `.pem`、`.key` — 加密密钥
- `secrets/` 目录
- `credentials/` 目录
...
**Kova 的哲学："你不是信任它会做对的事，你是建立一个系统让它只能做对的事。"**
