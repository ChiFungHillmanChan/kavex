...
**方法 B：修改 `hooks/kavex-loop.sh` 中的默认值**
...
设置 `KAVEX_LOOP_ACTIVE=1` 后，stop gate 会识别当前在 loop 迭代中，跳过交互检查。
...
这会关闭所有 Kavex hooks（stop gate、文件保护、命令拦截、自动格式化）。使用 `kavex activate` 重新启用。
...
模式使用不区分大小写的子串搜索进行匹配。Kavex 在匹配前会自动规范化引号和反斜杠转义字符，因此混淆变体也会被拦截。
...
Kavex 会自动检测 Codex 是否可用。你只需要安装它并设置 API 密钥。
...
完成。Kavex 的 `hooks/lib/codex-assist.sh` 会在运行时通过 `command -v codex` 检查可用性。当 Codex 可用时，会在反复失败后用于跨模型诊断和代码审查。

你还可以设置 `CODEX_TIMEOUT`（默认：120 秒）控制 Kavex 等待 Codex 响应的时间。
...
默认为每滚动小时 **100** 次调用。达到上限时，Kavex 会暂停并显示倒计时，等最旧的调用过期后自动继续。
