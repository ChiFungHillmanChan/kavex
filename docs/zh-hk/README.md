...
<p align="center">
  <img src="../../assets/kavex-hero.png" alt="Kavex — 自主工程協議" width="100%" />
</p>
...
Kavex 係一套「規則 + 腳本 + 指令」嘅組合包。你裝落去任何一個 project 入面，Claude Code 就會由一個「你問佢答」嘅 AI 助手，變成一個「自動做嘢、自動測試、自動改錯」嘅工程團隊。

**簡單講：裝咗 Kavex 之後，Claude 唔再問你「我應唔應該咁做？」，佢直接做，做完自己檢查，檢查唔過自己再改。**
...
```bash
claude /install kavex          # 輕量版：指令 + 技能
claude /install kavex-full     # 完整版：指令 + 技能 + hooks + 執行保障
```
...
|--------|--------|
| **kavex** | 斜線指令 + 工程協議技能 |
| **kavex-full** | kavex 所有嘢 + 安全 hooks、驗證閘門、自動格式化、commit 閘門、Team Loop |
...
<p align="center">
  <img src="../../assets/kavex-safety.png" alt="Kavex 7 層驗證架構" width="100%" />
</p>
...
完整嘅 7 層驗證係喺 **Team Loop**（`/kavex:loop`）入面透過 `verify-gate.sh` 跑嘅，唔係每次停都跑。包括：
...
<p align="center">
  <img src="../../assets/kavex-workflow.png" alt="Kavex Team Loop — 6 階段工作流程" width="100%" />
</p>

呢個係 Kavex 最強嘅嘢。你寫一個 PRD（Product Requirements Document，即係一個 to-do list），然後打：
...
**但佢唔會問你。** 佢會自己做假設，然後寫落去 `.kavex-loop/plans/item-N-clarify.md`。
...
4. 寫一個詳細計劃去 `.kavex-loop/plans/item-N-plan.md`
...
**如果任何一層 fail：**
- 將所有錯誤寫入 `.kavex-loop/current-failures.md`，包含 file:line 嘅具體位置
...
寫入 `.kavex-loop/current-review.md`，格式：
```
...
- `.kavex-loop/LOOP_PROGRESS.md` — 邊啲 item 完成咗、邊個正在做
- `.kavex-loop/ITERATION_LOG.md` — 每次 iteration 嘅詳細記錄

**STUCK 偵測：**
如果同一個 item 修咗 5 次（預設）都修唔到，Claude 會：
- 標記為 STUCK
- 寫入 `.kavex-loop/STUCK_ITEMS.md`
- 跳過，繼續下一個 item

**可以恢復：**
如果你中途停咗（斷線、手動取消等等），`.kavex-loop/` 入面有晒進度。下次再跑 `/kavex:loop docs/my-prd.md`，佢會問你：`resume`（繼續）/ `restart`（重新開始）/ `cancel`（取消）。
...
做大型功能（多個 item）：
  /kavex:loop docs/prd.md      <- 自動逐項實現，每項過 6 個 phase
...
<p align="center">
  <img src="../../assets/kavex-comparison.png" alt="冇 Kavex vs 有 Kavex" width="100%" />
</p>
...
**Kavex 嘅哲學：「你唔係信任佢會做啱嘢，你係建立一個系統令做錯嘢變得困難。」**
...
**Hooks 唔保證嘅嘢：**
- 用戶可以停用 hooks（`kavex deactivate` 或者改 settings.json）
