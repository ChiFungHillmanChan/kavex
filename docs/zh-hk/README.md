...
- [四個 Hooks（自動觸發）](#四個-hooks自動觸發)
  ...
  - [Hook 2：7 層閘門](#hook-2verify-on-stopsh--7-層閘門)
...
### Hook 2：`verify-on-stop.sh` — 7 層閘門

**幾時跑？** 每次 Claude 話「我做完啦」想停低嘅時候。

**做咩？** 跑 7 層檢查。前 6 層係「阻截型」——唔通過嘅話 Claude **唔可以停**，要繼續修。第 7 層只係警告。

一層一層咁講：

**第 1 層：Build（編譯）**
- 你個 project 要 compile 得到。
- Node.js → `npm run build`
- Go → `go build ./...`
- Rust → `cargo build`
- Java → `mvn compile`
- .NET → `dotnet build`
- 如果 build 唔到 → **阻截，Claude 唔准停**

**第 2 層：Unit Tests（單元測試）**
- 跑你所有嘅 unit test。
- 自動偵測測試框架：vitest、jest、pytest、go test、cargo test、rspec、mvn test……
- **Flaky test 處理：** 如果有 test fail，佢會自動重跑一次。第二次 pass 就算 pass（因為有啲 test 係間唔中 fail 嘅）。
- 如果重跑都 fail → **阻截**

**第 3 層：Integration Tests（整合測試）**
- 只有你個 project 有設定 `test:integration` 腳本先至會跑。
- 冇嘅話自動跳過。
- 有嘅話同 unit test 一樣，fail 會 retry 一次。

**第 4 層：E2E Tests（端到端測試）**
- 只有你裝咗 Playwright 先至會跑。
- 冇 Playwright → 自動跳過。
- 有嘅話跑 `npx playwright test`。
...
**第 7 層：Security Audit（安全審計）— 只係警告**
- `npm audit`、`pip-audit`、`cargo audit`……
- 有問題只會顯示警告，**唔會阻截**。

**即係話：** Claude 做完嘢，佢想停，但佢要過晒 6 關先至停得。如果有任何一關 fail，佢要自己修到過為止。
...
會保護嘅檔案包括：
- `.env`、`.env.production`、`.env.local` — 環境變數（通常有 API key）
- `.pem`、`.key` — 加密金鑰
- `secrets/` 資料夾
- `credentials/` 資料夾
...
**Kova 嘅哲學：「你唔係信任佢會做啱嘢，你係建立一個系統令佢只可以做啱嘢。」**
