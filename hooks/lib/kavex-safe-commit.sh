#!/bin/bash
# kavex-safe-commit.sh — Sourceable library for safe commit during kavex loop
# Source this file: source "$LIB_DIR/kavex-safe-commit.sh"
#
# Provides: kavex_safe_commit <item_num> <description> [no_commit]
...
#
# Prerequisites: kavex-snapshot.sh must be sourced (for working_tree_hash)
...
# Usage: _kavex_stage_changes <state_dir>
_kavex_stage_changes() {
  local state_dir="$1"
  ...
  comm -13 "$state_dir/.pre-untracked" "$state_dir/.post-untracked" 2>/dev/null | while IFS= read -r changed_file; do
    [ -z "$changed_file" ] && continue
    # Skip sensitive files and kavex state dir
  ...
  # Safety net: unstage any sensitive files and kavex state
  git reset HEAD -- '.kavex-loop/' '*.env' '*.env.*' '*.pem' '*.key' '*.p12' '*.pfx' '*.jks' \
    'secrets/' 'credentials/' '.secrets/' '.credentials/' >/dev/null 2>&1 || true
}

# Commit changes for a PRD item with safe staging.
# Usage: kavex_safe_commit <item_num> <description> [no_commit]
# Args:
...
kavex_safe_commit() {
  ...
  local state_dir="${4:-.kavex-loop}"

  # Guard: snapshot must exist and be recent (<10 min)
  if [ ! -f "$state_dir/.pre-tracked" ]; then
    echo "ERROR: No snapshot found at $state_dir/.pre-tracked. Run kavex_snapshot first." >&2
    return 1
  fi
  ...
  if [ "$snapshot_age" -gt 600 ]; then
    echo "ERROR: Snapshot is ${snapshot_age}s old (>600s). Re-run kavex_snapshot." >&2
    return 1
  fi
  ...
  _kavex_stage_changes "$state_dir"
