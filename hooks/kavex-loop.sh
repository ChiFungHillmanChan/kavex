#!/bin/bash
# kavex-loop.sh — Kavex Smart Loop orchestrator
# Usage: bash hooks/kavex-loop.sh <prd-file> [options]
...
source "$LIB_DIR/kavex-verify.sh"
...
parse_args() {
  ...
  if [ -z "$PRD_FILE" ]; then
    echo "Usage: kavex-loop.sh <prd-file> [--max-iterations N] [--max-fix-attempts N] [--no-commit] [--dry-run]" >&2
    exit 1
  fi
}
...
run_claude_with_prompt() {
  ...
  KAVEX_LOOP_ACTIVE=1 claude -p "$prompt_content" \
...
# provided by kavex-snapshot.sh and kavex-safe-commit.sh (sourced above).
# Legacy wrappers for backward compatibility within this script:
snapshot_pre_iteration() { kavex_snapshot "$STATE_DIR"; }
working_tree_hash() { git hash-object --no-filters -- "$1" 2>/dev/null || echo "__MISSING__"; }
stage_item_changes() { _kavex_stage_changes "$STATE_DIR"; }
...
Kavex Smart Loop — PRD item $item_num
PRD: $PRD_FILE
...
_acquire_loop_lock() {
  LOCK_DIR="${STATE_DIR}/.kavex-loop.lock"
  local retries=0
  while [ "$retries" -lt 3 ]; do
    ...
    # Lock exists — check if holder is alive AND is actually kavex-loop
    local stale_pid=""
    [ -f "$LOCK_DIR/pid" ] && stale_pid=$(cat "$LOCK_DIR/pid" 2>/dev/null)
    if [ -n "$stale_pid" ] && kill -0 "$stale_pid" 2>/dev/null; then
      local holder_cmd
      holder_cmd=$(ps -p "$stale_pid" -o args= 2>/dev/null || true)
      if echo "$holder_cmd" | grep -q "kavex-loop"; then
        echo "ERROR: Another Kavex loop is already running in this project (PID $stale_pid)." >&2
        exit 1
      fi
    fi
    # Stale lock (dead process or PID recycled to non-kavex) — remove and retry
    rm -rf "$LOCK_DIR"
    retries=$((retries + 1))
  done
...
main() {
  ...
  echo "" >&2
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" >&2
  echo " KAVEX SMART LOOP — COMPLETE" >&2
